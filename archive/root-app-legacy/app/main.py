...
from app.services.pendings import (
    list_assignment, assign_ride,
    list_yooga_groups, yooga_group_items, resolve_yooga
)
from app.services.seed import seed_weekly_couriers
from app.services.payouts import compute_week_payout_preview, close_week, pay_week, get_payout_snapshot
from app.services.ledger import list_week_ledger, create_ledger_entry, delete_ledger_entry
...
def couriers_put_payment(courier_id: str, body: CourierPaymentIn, db: Session = Depends(get_db)):
    p = upsert_payment(
        ...
        key_value_raw=body.key_value_raw,
        bank=body.bank,
    )
    return CourierPaymentOut(
        courier_id=str(p.courier_id),
        key_type=p.key_type,
        key_value_raw=p.key_value_raw,
        bank=p.bank,
...
def week_payout_csv(week_id: str, db: Session = Depends(get_db)):
    # If already closed, export snapshot; otherwise export preview.
    from app.services.payouts import get_week_or_404
    import csv
    import io
